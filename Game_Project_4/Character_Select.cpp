...
void Character_Select::receiveAdditionalCards(const std::vector<Card>& additionalCards) {
...
void Character_Select::enhance() {
    while (true) {
        cout << "강화할 스탯을 선택하여 주십시오 (현제 플레이어 골드:" << Character::Gold <<")" << endl;
        cout << endl <<"1. 공격력(+3)\t 2. HP(+10씩 누적증가)\t 3. 방어력(+2)\t 4. 스탯 확인 (강화종료 : 0) >>" << endl;
        int c;
        cin >> c;
        switch (c) {
        case 1:
